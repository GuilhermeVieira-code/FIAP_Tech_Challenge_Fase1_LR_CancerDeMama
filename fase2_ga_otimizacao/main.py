"""
main.py
Pipeline principal da Fase 2 — Otimização de Hiperparâmetros via AG.
...
Fluxo:
  ...
  4. Compara baseline vs. melhor modelo otimizado no conjunto de teste
  5. Gera explicação e relatórios via Gemini (opcional)
  6. Salva resultados e gráficos
...
from fase2_ga_otimizacao.src.model_evaluator import (
    load_data, evaluate_full, evaluate_genes, build_pipeline,
)
from fase2_ga_otimizacao.src.chromosome import baseline_genes, decode
from fase2_ga_otimizacao.src.genetic_algorithm import GAConfig, run_genetic_algorithm
...
EXPERIMENTS = [
    GAConfig(population_size=30,  n_generations=20, mutation_rate=0.15,
             mutation_sigma=0.15, cv_folds=5, seed=42),
    GAConfig(population_size=50,  n_generations=30, mutation_rate=0.10,
             mutation_sigma=0.12, cv_folds=5, seed=42),
    GAConfig(population_size=80,  n_generations=30, mutation_rate=0.20,
             mutation_sigma=0.18, cv_folds=5, seed=42),
]
EXPERIMENT_LABELS = [
...
def plot_convergence(results, labels):
    ...
    for ax, title in zip(axes, ["Melhor Fitness", "Fitness Médio"]):
        ...
        ax.set_ylabel("Fitness (maximizar)")
...
def plot_metrics_comparison(baseline, best_opt, best_label):
    ...
    b_vals    = [baseline["recall"], baseline["specificity"],
                 baseline["f1"], baseline["accuracy"]]
    opt_vals  = [best_opt["recall"],   best_opt["specificity"],
                 best_opt["f1"],       best_opt["accuracy"]]
...
def main(skip_llm: bool = False, cv_folds: int = 5):
    ...
    best_ga   = min(ga_results, key=lambda r: -r.best_individual.fitness)
    ...
    for res, label in zip(ga_results, EXPERIMENT_LABELS):
        best_metrics = evaluate_genes(res.best_individual.genes, X_train, y_train, cv_folds)
    ...
    if skip_llm:
        ...
        print("\n[5/5] Gerando relatórios via Gemini...")
        try:
            from fase2_ga_otimizacao.llm.report_generator import (
                explain_diagnosis, compare_models, analyze_experiments
            )

            # Explicação de diagnóstico (exemplo com primeiro paciente do teste)
            pipeline = best_opt["pipeline"]
            sample   = X_test[0:1]
            pred     = int(pipeline.predict(sample)[0])
            proba    = pipeline.predict_proba(sample)[0][1]

            # Features mais importantes (coeficientes da LogReg)
            ...
            if hasattr(pipeline, "named_steps") and "pca" in pipeline.named_steps:
                ...
                feature_names = [c for c in df_tmp.columns
                                 if c not in ("id", "Unnamed: 32", "diagnosis")]
            ...
            explain_diagnosis(
                ...
                recall=best_opt["recall"],
                specificity=best_opt["specificity"],
                f1_score=best_opt["f1"],
            ...
            exp_summary = [
                {
                    ...
                    "recall":          evaluate_genes(r.best_individual.genes, X_train, y_train)["recall"],
                    "specificity":     evaluate_genes(r.best_individual.genes, X_train, y_train)["specificity"],
                    "f1":              evaluate_genes(r.best_individual.genes, X_train, y_train)["f1"],
            ...
            print(f"[LLM] Erro: {e}")
            print("[LLM] Configure GEMINI_API_KEY no .env ou use --skip-llm")
    ...
    print(f"  - convergence.png          (convergência do AG)")
    print(f"  - metrics_comparison.png   (baseline vs. otimizado)")
    print(f"  - experiment_results.json  (métricas completas)")
    if not skip_llm:
        print(f"  - explicacao_diagnostico.txt (Gemini)")
        print(f"  - comparacao_modelos.txt     (Gemini)")
        print(f"  - analise_experimentos.txt   (Gemini)")
    print("=" * 60 + "\n")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Fase 2 — Otimização de Hiperparâmetros via AG"
    )
    parser.add_argument("--skip-llm",  action="store_true",
                        help="Pula chamadas à API Gemini")
    parser.add_argument("--cv-folds",  type=int, default=5,
                        help="Folds de validação cruzada (use 3 para testes rápidos)")
